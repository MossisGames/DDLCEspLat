        ...
        if not persistent.autoload or not main_menu:
            ...
            if main_menu:
                ...
                if persistent.playthrough == 1:
                    textbutton _("ŔŗñĮ¼»ŧþŀÂŻŕěōì«") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Por favor, introduce tu nombre", ok_action=Function(FinishEnterName)))
                else:
                    textbutton _("Nuevo juego") action If(persistent.playername, true=Start(), false=Show(screen="name_input", message="Por favor, introduce tu nombre", ok_action=Function(FinishEnterName)))
            ...
            if renpy.variant("pc"):
                ...
                textbutton _("Ayuda") action [Help("LEEME.html"), Show(screen="dialog", message="El archivo de ayuda se abrió en tu navegador.", ok_action=Hide("dialog"))]
...
init python:
    def FileActionMod(name, page=None, **kwargs):
        if persistent.playthrough == 1 and not persistent.deleted_saves and renpy.current_screen().screen_name[0] == "load" and FileLoadable(name):
            return Show(screen="dialog", message="Error: \"personajes/sayori.chr\"\n\nEl archivo está perdido o corrupto",
                ok_action=Show(screen="dialog", message="El archivo de guardado está roto. Comenzando nuevo juego.", ok_action=Function(renpy.full_restart, label="start")))
        elif persistent.playthrough == 3 and renpy.current_screen().screen_name[0] == "save":
            return Show(screen="dialog", message="Ya no hay razón para guardar.\nNo te preocupes, No iré a ningún lado.", ok_action=Hide("dialog"))
        else:
            ...
            grid gui.file_slot_cols gui.file_slot_rows:
                ...
                for i in range(gui.file_slot_cols * gui.file_slot_rows):
                    ...
                    button:
                        ...
                        text FileTime(slot, format=_("{#file_time}%A, %d de %B del %Y, %H:%M"), empty=_("Espacio vacío")):
        ...
        vbox:
            ...
            hbox:
                ...
                vbox:
                    style_prefix "check"
                    label _("Diálogos")
                    textbutton _("Subtitulado") action SetField(persistent, "idioma", "subesp")
                    textbutton _("Doblado") action SetField(persistent, "idioma", "doblado")
            ...
            hbox:
                ...
                vbox:
                    ...
                    label _("Tiempo de Espera Automático")

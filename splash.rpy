init python:
    menu_trans_time = 1
    splash_message_default = "Este juego no es adecuado para niños\n ni para aquellos que se alteran fácilmente."
    splash_messages = [
    "Eres mi solecito,\nMi único solecito",
    "Te extrañé.",
    "Juega conmigo",
    "Es sólo un juego, en su mayoría.",
    "¿Este juego no es adecuado para niños\n ni para aquellos que se alteran fácilmente?",
    "sdfasdklfgsdfgsgoinrfoenlvbd",
    "null",
    "Le tengo niños garantizados al infierno",
    "PM murió por esto.",
    "Fue sólo parcialmente tu culpa.",
    "Este juego no es adecuado para niños\n ni para aquellos que se desmiembran fácilmente.",
    "No olvides hacer una copia de seguridad del archivo de Monika."
    ]
...
image intro2:
    ...
    "white"
    0.5
    "bg/splash2.png" with Dissolve(0.5, alpha=True)
    2.5
    "white" with Dissolve(0.5, alpha=True)
    0.5
...
label splashscreen:
    ...
    if not firstrun:
        if persistent.first_run and (config.version == persistent.oldversion or persistent.autoload == "postcredits_loop"):
            ...
            scene black
            menu:
                "Se encontró un guardado previo. ¿Te gustaría eliminarlo y empezar de nuevo?"
    ...
    if not persistent.first_run:
        ...
        with Dissolve(1.0)
        pause 1.0
        "Este juego no es adecuado para niños ni para aquellos que se alteran fácilmente."
        "Las personas que sufren de ansiedad o depresión pueden no tener una buena experiencia jugando este juego. Para advertencias de contenido, abra el archivo advertencia.html"
        menu:
            "Al jugar Doki Doki Literature Club!, usted acepta que tiene al menos 13 años de edad y acepta su exposición a contenido altamente perturbador."
            "De acuerdo.":
                pass
    ...
    python:
        ...
        if persistent.playthrough == 0:
            try: renpy.file("../personajes/sayori.chr")
            except: s_kill_early = True
        if not s_kill_early:
            if persistent.playthrough <= 2 and persistent.playthrough != 0:
                try: renpy.file("../personajes/monika.chr")
                except: open(config.basedir + "/personajes/monika.chr", "wb").write(renpy.file("monika.chr").read())
            if persistent.playthrough <= 1 or persistent.playthrough == 4:
                try: renpy.file("../personajes/natsuki.chr")
                except: open(config.basedir + "/personajes/natsuki.chr", "wb").write(renpy.file("natsuki.chr").read())
                try: renpy.file("../personajes/yuri.chr")
                except: open(config.basedir + "/personajes/yuri.chr", "wb").write(renpy.file("yuri.chr").read())
            if persistent.playthrough == 4:
                try: renpy.file("../personajes/sayori.chr")
                except: open(config.basedir + "/personajes/sayori.chr", "wb").write(renpy.file("sayori.chr").read())
    ...
    if s_kill_early:
        ...
        show noise:
            alpha 0.1
        with Dissolve(1.0)
        show expression Text("Ahora todos pueden ser felices.", style="sayori_text"):
            xalign 0.8
            yalign 0.5
            alpha 0.0
            600
...
label after_load:
    ...
    if persistent.yuri_kill > 0 and persistent.autoload == "yuri_kill_2":
        ...
        if persistent.playername == "":
            m "Eres tan divertido."
        else:
            m "Eres tan divertido, [persistent.playername]."
        $ renpy.utter_restart()
    else:
        if persistent.playthrough == 0 and not persistent.first_load and not config.developer:
            $ persistent.first_load = True
            call screen dialog("Consejo: Puedes usar el botón \"Saltar\" para \nsaltar texto que ya viste.", ok_action=Return())
    return
